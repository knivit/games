...
public class InputService {
    ...
    static int[] s_keymapIgnore = new int[] {30, ',', '9', ':', '<', '>', '@', 'Z', 128}; /*!< Keys to ignore when reading. */
    ...
    static int[] s_keymapSpecialMask = new int[] {0x00, 0x00, 0xFE, 0x87, 0xFF, 0xC0, 0x1F, 0x00};
    ...
    static int[] s_keymapNormal = new int[] {
    ...
    static int[] s_keymapShift = new int[] {
        0, '~', '!', '@', '#', '$', '%', '^', '&', '*', '(',  ')', '_', '+',   0,   8,
    ...
    static int[] s_keymapNumlock = new int[] {0, '7', '4', '1',   0, '/', '8', '5', '2', '0', '*', '9', '6', '3', '.', '-', '+', 0, '\r', 0};
    ...
    static int[] s_translateExtendedMap = new int[] {'8', 29, 'R', 'S', 'K', 'G', 'O', 'H', 'P', 'I', 'Q', 'M', '5', 28, '7', 'F'};
    ...
    static int[] s_translateMap = new int[] {'>', '@', 'K', 'L', 'O', 'P', 'Q', 'S', 'T', 'U', 'V', 'Y', '_', 'l', '|', 0};
    ...
    static int[] s_translateTo = new int[] {'<', ':', 'c', 'h', '\\', '[', ']', '`', 'b', 'e', 'g', 'f', '7', '+', '|', 0};
     ...
     * 0x58 => 0x7b            F12
     */
    static int[] s_keyTranslate = new int[] {
     ...
     */
    static int Input_Keyboard_Translate(int keyValue) {
        int i;

        if ((g_inputFlags & INPUT_FLAG_NO_TRANSLATE) != 0) return keyValue;

        for (i = 0; i < s_translateMap.length; i++) {
            if (s_translateMap[i] != (int)(keyValue & 0xFF)) continue;
     ...
     */
    static int Input_History_Add(int value) {
        int index;
     ...
     */
    static int Input_AddHistory(int value) {
        ...
        if ((flags & INPUT_FLAG_MOUSE_EMUL) != 0 && (input & 0x400) == 0) {
            ...
            if (((flags & INPUT_FLAG_KBD_MOUSE_CLK) != 0 && (value == 0x2B || value == 0x3D || value == 0x6C)) || value == 0x63) {
                ...
                if ((input & 0x200) != 0) {
                    ...
                    int[] data_0AB6 = new int[] {
                        8, 2, 8, 6, 4, 3, 8, 5,
                        8, 8, 8, 8, 0, 1, 8, 7
                    };

                    /* Copied from 29E8:000A - 29E8:002E */
                    XYPosition[] mousePos = new XYPosition[] {
                        new XYPosition(0x0a0, 0x000), new XYPosition(0x13f, 0x000), new XYPosition(0x13f, 0x045), new XYPosition(0x13f, 0x089),
                        new XYPosition(0x0a0, 0x089), new XYPosition(0x000, 0x089), new XYPosition(0x000, 0x045), new XYPosition(0x000, 0x000),
                        new XYPosition(0x0a0, 0x045) /* center position */
                    ...
                    int[] offsetSmall = new int[] { -1, 0,  1};	/* When shift is released : SLOW */
                    int[] offsetBig = new int[] {-16, 0, 16};	/* When shift is pressed : FAST */
				    int[] change;

                    change = ((input & 0x100) == 0) ? offsetSmall : offsetBig;

                    inputMouseX += change[1 + dx];
                    inputMouseY += change[1 + dy];
                    if (inputMouseX >= 0x8000) inputMouseX = 0;
                    if (inputMouseY >= 0x8000) inputMouseY = 0;
                    if ((int)inputMouseX > SCREEN_WIDTH - 1)  inputMouseX = SCREEN_WIDTH - 1;
                    if ((int)inputMouseY > SCREEN_HEIGHT - 1) inputMouseY = SCREEN_HEIGHT - 1;
                }
